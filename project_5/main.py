...
class TrendlineVisualizer(object):
    ...
    def save_state_csv(self, trend_label, item_list, file_name, append_date_to_end_of_file):
        ...
        with open("data.csv", file_option, newline = "") as f:
            w = csv.writer(f, delimiter=',')
            if file_option == "w":
                w.writerow(('label', 'date', 'count'))
            for i in item_list:
                w.writerow((trend_label, i['date'], i['count'] ))
    ...
    def get_bills_mentioning_topic(self, search_term, trend_label, make_multiple_queries=False):
        ...
        self.save_state_csv(trend_label, results, 'data.csv', append_date_to_end_of_file = make_multiple_queries)


trends = TrendlineVisualizer()
trends.get_bills_mentioning_topic(search_term = "woman AND abortion AND ban", trend_label = "Woman For Ban")
trends.get_bills_mentioning_topic(search_term = "women AND abortion", trend_label = "Women and Abortion", make_multiple_queries=True)
trends.get_bills_mentioning_topic(search_term = "men AND abortion ban", trend_label = "Men For Ban", make_multiple_queries=True)
#trends.get_bills_mentioning_topic(search_term = "abortion ban", tend)
trends.get_bills_mentioning_topic(search_term = "men AND abortion", trend_label = "Men and Abortion", make_multiple_queries=True)
trends.get_bills_mentioning_topic(search_term = "abortion ban", trend_label = "Abortion Ban", make_multiple_queries=True)
